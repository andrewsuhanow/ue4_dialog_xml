...
void SDLGButtonCondition::Construct(const FArguments& InArgs)
{


	Delegate_OnDLGButtonConditionClicked = InArgs._OnDLGButtonConditionClicked;
	Param_ButtonConditionType = InArgs._ButtonConditionType.Get();
	...
	[
		SNew(SButton)
		.OnClicked(this, &SDLGButtonCondition::SendData)
	];
}
...
FReply SDLGButtonCondition::SendData()
{
	GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Green, FString::SanitizeFloat(111222333));

	Delegate_OnDLGButtonConditionClicked.Execute(Param_ButtonConditionType, Param_DialodTag);
...
//void SDLGButtonCondition::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
//{
//}
